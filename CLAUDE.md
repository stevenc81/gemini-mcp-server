...
- **gemini.py** — Runs `gemini -p <prompt> -o stream-json` as a subprocess. Pipes file context via stdin to leverage Gemini's large context window. Supports `--resume <session_id>` for conversation continuity (dropped on model fallback). Uses stream-json output with a two-phase timeout: during model thinking (before first response token), only the hard wall-clock timeout (default 300s) applies; once streaming starts, `_STREAMING_TIMEOUT` (90s) between output lines catches stalled connections. Parses streamed events (init, message, result) to extract response text and stats (model, tokens, session ID) into a metadata footer.
